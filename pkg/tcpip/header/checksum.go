...
// ChecksumUpdateUint16 updates a uint16 value in a calculated checksum.
//
// The value MUST begin at a 2-byte boundary in the original buffer.
func ChecksumUpdateUint16(xsum, old, new uint16) uint16 {
...
// ChecksumUpdateAddress updates an address in a calculated checksum.
//
...
func ChecksumUpdateAddress(xsum uint16, old, new tcpip.Address) uint16 {
	const uint16Bytes = 2
	...
	//                C' = C + (-m) + m' = C + (m' - m)
	for len(old) != 0 {
		xsum = ChecksumUpdateUint16(xsum, (uint16(old[0])<<8)+uint16(old[1]), (uint16(new[0])<<8)+uint16(new[1]))
		old = old[uint16Bytes:]
		new = new[uint16Bytes:]
	}
